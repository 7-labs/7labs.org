...
Run dependency audit as a separate review step when you need a supply-chain report:

```bash
npm audit --audit-level=moderate
RUN_NPM_AUDIT=true ./deploy.sh cf-build
```
...
- `./deploy.sh cf-build`: runs the full production validation chain and OpenNext Cloudflare bundle generation.
- `RUN_NPM_AUDIT=true ./deploy.sh cf-build`: adds `npm audit` as an explicit extra gate when the current dependency graph is expected to pass it.
